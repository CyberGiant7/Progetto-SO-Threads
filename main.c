 ...
 * @gioco e @AreaGioco ,da cui vengono generati e uccisi tutti i thread, vengono chiamate le funzioni di stampa e
 * collisione. Inoltre vengono gestite le vite e l'eventuale vincita del gioco.
 */
...
StatoCorrente gioco() {
    ...
    int i, temp;                // Variabile contatore
    ...
    /// Thread nemici
    for (i = 0; i < M; ++i) {
	enemies[i].old_pos.x = -1;
    	enemies[i].old_pos.y = -1;
    	enemies[i].id = ID_NEMICO;
        enemies[i].index = i;                       // Assegnamendo dell'indice
	enemies[i].vite = 3;
	enemies[i].pos.y = (1 + (enemies[i].index % num_righe) * (DIM_NEMICO + 1));
        temp = (int) (enemies[i].index / num_righe) + 1;
        enemies[i].pos.x = maxx - temp * (DIM_NEMICO + 1);
...
_Bool AreaGioco(Oggetto *navicella, Oggetto *enemies, _Bool *sparo) {
    ...
    do {    // Inizio del ciclo
        ...
        *sparo = false;                     // e lo rendo false
        ...
        punteggio_f = (punteggio - (nemici_vivi * 100)) + (navicella->vite * 200);
        pthread_mutex_unlock(&mtx_nave);
        j++;
        usleep(10);
        refresh();
